...
int is_w_empty(struct seq_msg wnd[])
//return 1 if empty, return 0 if not empty
{
  for(int i = 0; i < WNDSIZE; i++)
    if (wnd[i].seq != -1) return 0; //some stuff still in here
  return 1;
}
...
int main(int argc, char *argv[])
{
  ...
  //send sync msg
  int duplicate = 0; int final_seq = -1;
  do {
  ...
  do {
    ...
    if(flags&FIN);
    else printf("Receiving packet %d\n",seq);
